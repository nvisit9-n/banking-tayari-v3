...
export function buildQuestion(qNum: number, setId: number, item: MasterItem): RawSangathitQuestion {
  // Uniform pseudo-random key distribution across A, B, C, D (0, 1, 2, 3)
  ...
  for (let slot = 0; slot < 4; slot++) {
    if (slot === targetIndex) {
      // Pure option text without pre-baked (A)/(B) tags to eliminate hints
      options.push(item.correct.replace(/^[\(\[]?[A-D][\)\]\.\:]\s*/i, '').trim());
    } else {
      options.push(item.distractors[dPtr % item.distractors.length].replace(/^[\(\[]?[A-D][\)\]\.\:]\s*/i, '').trim());
      dPtr++;
    }
  }
...
export function buildSingleLang(qNum: number, setId: number, item: SingleLangItem): RawSangathitQuestion {
  // Uniform pseudo-random key distribution across A, B, C, D (0, 1, 2, 3)
  const targetIndex = ((setId * 11) + (qNum * 5) + ((setId + qNum) % 4)) % 4;
  const options: string[] = [];
  let dPtr = 0;
...
const S12_POOL: MasterItem[] = [
  {
    ...
    expEng: "The Policy Rate under NRB's monetary policy corridor is maintained at 5.0% (with Bank Rate at 6.5% and Deposit Collection Rate at 3.0%).",
    expNep: "नेपाल राष्ट्र बैंकको अद्यावधिक मौद्रिक नीति अनुसार नीतिगत दर ५.०%, बैंक दर ६.५% र निक्षेप संकलन दर ३.०% कायम गरिएको छ।"
  },
  {
...
const S17_POOL: MasterItem[] = [
  {
    ...
    expNep: "नेपालको संविधानको भाग ३ मा धारा १६ देखि धारा ४६ सम्म कुल ३१ वटा मौलिक हकहरू रहेका छन्।"
  },
  {
...
const S19_POOL: MasterItem[] = [
  {
    ...
    expNep: "संविधानको धारा ८४ बमोजिम प्रतिनिधि सभामा २७५ सदस्य रहनेछन्।"
  },
  {
    ...
    expNep: "धारा ८६ अनुसार राष्ट्रिय सभा ५९ सदस्यीय हुनेछ।"
  }
];
...
const S20_POOL: MasterItem[] = [
  {
    ...
    distractors: ["Part 20 / भाग २०", "Part 22 / भाग २२", "Part 23 / भाग २३"],
...
const S22_POOL: MasterItem[] = [
  {
    ...
    expNep: "संयुक्त राष्ट्रसंघको स्थापना सन् १९४५ अक्टोबर २४ मा भएको थियो।"
  },
  {
    ...
    expEng: "Nepal was admitted to the UN on 14 December 1955 as the 72nd member.",
...
const S25_POOL: MasterItem[] = [
  {
    ...
    expEng: "Nepal became the 147th member of the WTO on 23 April 2004.",
...
const S28_POOL: MasterItem[] = [
  {
    ...
    expEng: "IPv4 uses 32-bit addresses formatted as four octets, whereas IPv6 uses 128-bit addresses.",
  ...
  },
  {
    ...
    expEng: "DNS acts as the phonebook of the Internet, translating human-friendly names into numeric IP addresses.",
...
const S29_POOL: MasterItem[] = [
  ...
  },
  {
    ...
    expEng: "LLM stands for Large Language Model, an AI algorithm trained on vast text data to understand and generate human-like language.",
...
const S30_POOL: MasterItem[] = [
  {
    ...
    distractors: ["Scurvy (स्कर्भी)", "Rickets (रिकेट्स)", "Beriberi (बेरीबेरी)"],
  ...
  },
  {
    ...
    expEng: "Palesha Goverdhan secured Nepal's first-ever Paralympic bronze medal in Para-Taekwondo (K44-57kg) in Paris 2024.",
...
const S31_POOL: MasterItem[] = [
  {
    ...
    expNep: "लुथर गुलिकले व्यवस्थापनका कार्यहरूलाई POSDCORB को रूपमा प्रस्तुत गरेका हुन्।"
  }
];
...
const S32_POOL: MasterItem[] = [
  {
    ...
    expNep: "कार्यालयमा प्राप्त भएका पत्रहरूलाई दर्ता किताबमा क्रमानुसार चढाउने कार्यलाई दर्ता भनिन्छ।"
  },
  {
...
const S36_POOL: MasterItem[] = [
  ...
  },
  {
    ...
    distractors: ["Rastriya Banijya Bank / राष्ट्रिय वाणिज्य बैंक", "Agricultural Development Bank / कृषि विकास बैंक", "Nepal Rastra Bank / नेपाल राष्ट्र बैंक"],
...
const S39_POOL: MasterItem[] = [
  {
    ...
    expNep: "सार्वजनिक-निजी साझेदारीमा BOOT को पूर्ण रूप Build, Own, Operate, Transfer हो।"
  }
];
...
function generateMathSlot41(setId: number): MasterItem {
  const cp = 400 + ((setId * 17) % 30) * 20;
  const profitPct = 10 + ((setId * 7) % 5) * 5;
...
function generateMathSlot42(setId: number): MasterItem {
  const p = 5000 + ((setId * 13) % 20) * 1000;
  const r = 5 + ((setId * 2) % 4) * 2;
  const t = 2 + (setId % 3);
  const si = (p * r * t) / 100;
...
function generateMathSlot43(setId: number): MasterItem {
  const r1 = 2 + (setId % 3);
  const r2 = 3 + ((setId * 2) % 3);
  const unit = 200 + ((setId * 11) % 15) * 50;
...
function generateMathSlot44(setId: number): MasterItem {
  const pairs = [
  ...
  return {
    ...
    expEng: `1 day's combined work = 1/${choice.a} + 1/${choice.b} = 1/${choice.together}. Thus, total time = ${choice.together} days.`,
    expNep: `दुवैको १ दिनको काम = १/${choice.a} + १/${choice.b} = १/${choice.together}। तसर्थ दुवै मिलेर गर्दा लाग्ने समय = ${choice.together} दिन।`
  };
}

function generateMathSlot45(setId: number): MasterItem {
  const probVariants: MasterItem[] = [
    {
...
const S50_POOL: SingleLangItem[] = [
  ...
  },
  {
    ...
    explanation: "कुनै वस्तु छुटिने वा अलग हुने आधार बुझाउने शब्द अपादान कारक हुन्छ र यसमा पञ्चमी विभक्ति लाग्दछ।"
  }
];
